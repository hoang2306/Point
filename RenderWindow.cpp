...
void RenderWindow::drawObject(Me* obj)
{
    ...
    */
    ...
    SDL_Rect rect;
    rect.x = obj->getPosn().x - 10;
    rect.y = obj->getPosn().y - 10;
    rect.w = 32;
    rect.h = 32;

    SDL_Texture* playerTex;
    SDL_Surface* tmpSurface = IMG_Load("texture/Ghost1.png");
...
{
    SDL_Texture* playerTex;
    SDL_Surface* tmpSurface = IMG_Load("texture/Spider2.png");
    playerTex = SDL_CreateTextureFromSurface(renderer, tmpSurface);
     SDL_FreeSurface(tmpSurface);
    for(auto it: enemyList)
    {
        SDL_Rect rect;
        rect.x = it->getPosn().x - 10;
        rect.y = it->getPosn().y - 10;
        rect.w = it->getLen();
        rect.h = it->getLen();
...
{
    SDL_Texture* playerTex;
    SDL_Surface* tmpSurface = IMG_Load("texture/Gold_1.png");
    playerTex = SDL_CreateTextureFromSurface(renderer, tmpSurface);
    SDL_FreeSurface(tmpSurface);
...
void RenderWindow::main_menu(LTexture* TextTexture1, LTexture* TextTexture2, LTexture* TextTexture3)
{
    ...
    TTF_Font* Font1 = TTF_OpenFont("font/halloween.otf", 52);
    TTF_Font* Font2 = TTF_OpenFont("font/halloween.otf", 38);

    TextTexture1->loadFromRenderedText(" POINT ", textColor1, renderer, Font1);
...
void RenderWindow::score_menu(int TotalScore, LTexture* TextTexture1, LTexture* TextTexture2)
{
    ...
    TTF_Font* Font = TTF_OpenFont("font/halloween.otf", 26);
...
void RenderWindow::end_menu(int TotalScore, set<int> ScoreSet, LTexture* TextTexture, LTexture* TextTexture1, LTexture* TextTexture2, LTexture* TextTexture3, LTexture* TextTexture4, LTexture* TextTexture5, LTexture* TextTexture6, LTexture* TextTexture61, LTexture* TextTexture62, LTexture* TextTexture63)
{
    ...
    TTF_Font* Font = TTF_OpenFont("font/halloween.otf", 40);
    TTF_Font* Font1 = TTF_OpenFont("font/halloween.otf", 52);
    TTF_Font* Font2 = TTF_OpenFont("font/halloween.otf", 24);
    TTF_Font* Font3 = TTF_OpenFont("font/halloween.otf", 36);
